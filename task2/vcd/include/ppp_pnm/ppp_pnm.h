...
extern uint8_t *ppp_pnm_read_part(const char *name, enum pnm_kind *kind, int *rows, int *columns, int *maxval,
                                  uint8_t *(*partfn)(enum pnm_kind kind, int rows, int columns, int *offset,
                                                     int *length));
 ...
 */
extern int ppp_pnm_write(const char *name, enum pnm_kind kind, int rows, int columns, int maxval, uint8_t *image);
