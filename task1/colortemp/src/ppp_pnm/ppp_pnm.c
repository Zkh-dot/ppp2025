 ...
 */
static int ppp_pnm_params(FILE *fd, enum pnm_kind *kind, int *rows, int *cols, int *maxval) {
    if (fgetc(fd) != 'P') {
        return -1;
    }
 ...
 */
static int ppp_pnm_read_data(FILE *fd, enum pnm_kind kind, int rows, int columns, int maxval, uint8_t *image,
    ...
    case PNM_KIND_PPM:
        int r = fseek(fd, 3 * offset, SEEK_CUR);
        printf("fseek ofset: %d, res = %d\n", offset * 3, r);
        length *= 3;
        printf("----> %d %d %d %d, offset: %d\n", image, sizeof(*image), length, fd, offset);
        size_t res = fread(image, sizeof(*image), length, fd);
        printf("-----> %d\n", res);
        fseek(fd, 0L, SEEK_END);
        int sz = ftell(fd);
        printf("file size ----->%d / %d = %d\n", offset*3 + length, sz,  sz-(offset*3 + length));
        if (res != length) {
            printf("ERROR\n");
            return -1;
        }
 ...
 */
static uint8_t *read_all(enum pnm_kind kind, int rows, int columns, int *offset, int *length) {
    int valuesPerPixel;
    *offset = 0;
    *length = rows * columns;
...
static uint8_t *read_part(enum pnm_kind kind, int rows, int columns, int *offset, int *length) {
    int valuesPerPixel;
    valuesPerPixel = kind == PNM_KIND_PPM ? 3 : 1;
    return (uint8_t *)malloc(sizeof(uint8_t) * *length * valuesPerPixel);
}
 ...
 */
extern uint8_t *ppp_pnm_read(const char *name, enum pnm_kind *kind, int *rows, int *columns, int *maxval) {
    return ppp_pnm_read_part(name, kind, rows, columns, maxval, read_all);
}

extern uint8_t *ppp_pnm_read_part(const char *name, enum pnm_kind *kind, int *rows, int *columns, int *maxval,
                                  uint8_t *(*part)(enum pnm_kind, int, int, int *, int *)) {
    FILE *fd;
    uint8_t *image;
    int offset, length;

    fd = fopen(name, "r");
    printf("fd after fopen: %d\n", fd);
    if (fd != NULL) {
        if (ppp_pnm_params(fd, kind, rows, columns, maxval) == 0) {
            if (*maxval < 256) {
                image = part(*kind, *rows, *columns, &offset, &length);
                if (image != NULL) {
                    if (ppp_pnm_read_data(fd, *kind, *rows, *columns, *maxval, image, offset, length) == 0) {
                        fclose(fd);
                        return image;
                    }
                    free(image);
...
uint8_t *api_read_part(const char *name, enum pnm_kind *kind, int *rows, int *columns, int *maxval) {
    return ppp_pnm_read_part(name, kind, rows, columns, maxval, read_part);
}
...
extern int ppp_pnm_write(const char *name, enum pnm_kind kind, int rows, int cols, int maxval, uint8_t *image) {
    ...
    case PNM_KIND_PGM:
        fprintf(fd, "P5\n%d %d\n%d\n", cols, rows, maxval);
        len = rows * cols;
        if (fwrite(image, sizeof(*image), len, fd) < len) {
            fclose(fd);
            return -1;
        }
        break;
